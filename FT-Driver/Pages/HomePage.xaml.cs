...
namespace FT_Driver.Pages
{
    public partial class HomePage : PhoneApplicationPage
    {
        ...
        private async void GetCurrentCorrdinate()
        {
            ...
            //fiveStepBeforeLat = driverFirstGeoposition.Coordinate.ToGeoCoordinate().Latitude;
            //fiveStepBeforeLng = driverFirstGeoposition.Coordinate.ToGeoCoordinate().Longitude;
        ...
        private void geolocator_PositionChanged(Geolocator sender, PositionChangedEventArgs args)
        {
            ...
            Deployment.Current.Dispatcher.BeginInvoke(() =>
            {
                ...
                map_DriverMap.SetView(geocoordinate.ToGeoCoordinate(), 16, MapAnimationKind.Linear);
                ...
                if (countForUpdateLocation > 2)
                {
                    Debug.WriteLine("Cập nhật vị trí Driver lên Server"); //DELETE AFTER FINISH
                    UpdateCurrentLocation(geocoordinate.ToGeoCoordinate().Latitude, geocoordinate.ToGeoCoordinate().Longitude);
                    countForUpdateLocation = 0;
                }
                ...
                if (isTrack)
                {
                    SetEndCoordinateOfTrip(geocoordinate.ToGeoCoordinate().Latitude, geocoordinate.ToGeoCoordinate().Longitude);
                    //CalculateTripDistanceAndCost();
                }

                //Vẽ đường Tracking map trong trường hợp đã nhấn nút Start Trip
                if (isTrackingRoute == true && isFinishTrip == false)
                {
                    //Lấy điểm cuối
                    fiveStepAfterLat = geocoordinate.ToGeoCoordinate().Latitude;
                    fiveStepAfterLng = geocoordinate.ToGeoCoordinate().Longitude;
                    ...
                    if (fiveStepBeforeLat != 0 && fiveStepAfterLat != 0)
                    {
                    ...
                    fiveStepAfterLat = fiveStepAfterLng = 0;
        ...
        private void TripTrackingOnMap()
        {
            ...
            map_DriverMap.MapElements.Add(_line);
        ...
        {
            _watcher.Stop();
            map_DriverMap.MapElements.Remove(_line);
        }
        ...
        private void SetViewAtHomeState()
        {
            ...
            grv_AcceptReject.Visibility = Visibility.Collapsed;
            grv_AcceptRejectbtn.Visibility = Visibility.Visible;
        ...
        {
            grv_ProcessScreen.Visibility = Visibility.Visible; //Disable process bar
        }
